...
5. [Detector 1 — IP Rate Limiter](#5-detector-1--ip-rate-limiter)
6. [Detector 2 — User-Agent Detection](#6-detector-2--user-agent-detection)
7. [Detector 3 — Header Fingerprint](#7-detector-3--header-fingerprint)
8. [Detector 4 — Credential Stuffing](#8-detector-4--credential-stuffing)
9. [Detector 5 — JS Challenge](#9-detector-5--js-challenge)
10. [Detector 6 — CAPTCHA](#10-detector-6--captcha)
11. [응답 생성 — challenge/mod.rs](#11-응답-생성--challengemodrs)
12. [메인 서버 — main.rs](#12-메인-서버--mainrs)
13. [Kafka 이벤트 스트리밍](#13-kafka-이벤트-스트리밍)
14. [동시성 모델](#14-동시성-모델)
15. [보안 설계 포인트](#15-보안-설계-포인트)
16. [테스트 전략](#16-테스트-전략)
...
[rust-engine:8080]  ←── 이곳이 WAF의 핵심
  ...
  ⑥ Block/Challenge/Captcha면 Kafka로 이벤트 전송 (비동기)
     │
     ├─[Pass]──→ [backend:80]  실제 서비스
     │
     └─[Kafka]  waf-events 토픽 → SIEM/분석 시스템
```
...
실제 서비스 서버. 이 프로젝트에서는 테스트용 nginx static 서버로 대체했다. Pass된 요청만 도달한다.
...
모든 Detector는 `&Packet`만 받는다. 소유권을 빼앗지 않고 읽기만 한다. 파이프라인에서 6개 Detector가 동일한 Packet을 순서대로 읽는다.
...
Block은 early exit이다. IpRateLimiter가 Block을 반환하면 나머지 5개 Detector는 실행하지 않는다.
...
## 6. Detector 2 — User-Agent Detection
...
## 7. Detector 3 — Header Fingerprint
...
## 8. Detector 4 — Credential Stuffing

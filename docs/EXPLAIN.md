...
`/alice/my-repo.git` 경로는 `.git`이 포함되지 않아 통과한다. `/.git`(루트의 숨김 폴더)만 차단한다.
...
JA3는 TLS 레이어에서 계산되므로 **nginx-module-ja3 또는 OpenResty + HTTPS 설정**이 필요하다. 현재 시스템은 HTTP(80포트)만 운영 중이므로 `X-JA3-Fingerprint` 헤더가 전달되지 않아 이 Detector는 항상 Pass를 반환한다. HTTPS 전환 후 활성화된다.
